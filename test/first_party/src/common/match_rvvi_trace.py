#!/usr/bin/env python3
"""Match a main-window TRACE sample stream against a field expect file."""
...
def match_events(
    events: list[dict[str, Any]], expect: dict[str, Any], xlen: int
) -> list[str]:
    specs = [s for s in expect.get("events", []) if when_ok(s.get("when"), xlen)]
    errors: list[str] = []
    i = 0
    for si, spec in enumerate(specs):
        ...
        if found is None:
            errors.append(
                f"expect[{si}] {spec.get('event', 'RET')} {spec['match']}: not found in window"
            )
            continue
...
def match_text(text: str, expect: dict[str, Any], elf: str | None) -> list[str]:
    params, events = parse_trace(text)
    xlen = int(params.get("XLEN", "64"))
    start_name = expect.get("window", "main")
    if elf:
    ...
    return match_events(events, expect, xlen)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Match RVVI-TEXT against TRACE field expects"
    )
    ...
    with open(args.expect, encoding="utf-8") as f:
        expect = json.load(f)
    errors = match_text(text, expect, args.elf)
    if errors:
